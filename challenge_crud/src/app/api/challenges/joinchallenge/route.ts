...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!employee || !challenge) {
      return NextResponse.json({ success: false, message: "Employee or challenge not found" });
    }
    ...
    if (challenge.teamMembers.length >= challenge.maxTeamMembers) {
      return NextResponse.json({ success: false, message: "This challenge has reached max no. of team members." });
    }
    ...
    return NextResponse.json({ success: true, message: "Successfully joined the challenge." });
  } catch (error:any) {
    console.log(error);
    return NextResponse.json({ success: false, message: "Error joining the challenge." });
  }
}
