...
export async function GET(request: NextRequest) {
  try {
    ...
    const employee = await Employee.findById(employeeId).populate('selectedChallenges');
    console.log();
    ...
    console.log(error);
    return NextResponse.json({ success: false, message: "Error fetching selected challenges" });
  }
}
